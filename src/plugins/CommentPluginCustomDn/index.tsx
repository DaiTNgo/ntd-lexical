...
function CommentsPanelList({
  ...
  const $handleClickThread = (id: Comment['id'] | Thread['id']) => () => {
  ...
  const currentThread = comments.find((commentOrThread) => {
    return commentOrThread.id === activeIDs[0];
  });

  /*TODO: Start Modal Comment */
  const [content, setContent] = useState('');
  const [canSubmit, setCanSubmit] = useState(false);
  const boxRef = useRef<HTMLDivElement>(null);
  const selectionState = useMemo(
  ...
  const updateLocation = useCallback(() => {
    editor.getEditorState().read(() => {
      ...
      console.log('$isRangeSelection(selection)', $isRangeSelection(selection));
      if ($isRangeSelection(selection)) {
        const anchor = selection.anchor;
        const focus = selection.focus;
        const range = createDOMRange(
  ...
  }, [editor, selectionState, currentThread]);
  ...
  }, [selectionState.container, updateLocation, currentThread]);
  ...
  }, [updateLocation, currentThread]);

  const [isOptionComment, setIsOptionComment] = useState(false);
  ...
  const reset = () => {
    setIsEdit(false);
    setIsOptionComment(false);
  };

  console.log(
    'currentThread',
    currentThread && currentThread.type === 'thread',
    isOptionComment,
  );
  if (currentThread && currentThread.type === 'thread') {
    return createPortal(
      <div
        ref={boxRef}
        ...
        {isEdit ? (
          <>
            <textarea
              ...
              className="CommentPlugin_CommentInputBox_Editor_Edit">
              {content}
            </textarea>
            ...
            <div
              ...
              <Button
                onClick={() => {
                  currentThread.comments[0].content = content;
                  reset();
                }}
                disabled={!content}
                className="CommentPlugin_CommentInputBox_Button primary">
              ...
              {isOptionComment && (
                <div
                  style={{
                    ...
                    borderRadius: 4,
                    boxShadow:
                      '0 0 0 1px rgba(0,0,0,.1), 0 1px 2px rgba(0,0,0,.2)',
                  }}>
                  <div
                    onClick={() => {
                      // setTimeout(() => {
                      ...
                      setIsEdit(true);
                      setContent(currentThread.comments[0].content);
                    }}>
                    Edit
                  </div>
                  <div
                    onClick={() => {
                      deleteCommentOrThread(currentThread);
                      setIsOptionComment(false);
                    }}>
                    Delete
                  </div>
